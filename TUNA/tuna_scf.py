...
4. Functions for convergence acceleration (apply_DIIS, apply_damping, etc.)
...
def calculate_exchange_matrix(P: ndarray, ERI_AO: ndarray) -> ndarray:
...
def construct_density_matrix(molecular_orbitals: ndarray, n_occ: int, n_electrons_per_orbital: int) -> ndarray:
...
def diagonalise_Fock_matrix(F: ndarray, X: ndarray) -> tuple[ndarray, ndarray]:
...
def calculate_SCF_changes(E: float, E_old: float, P: ndarray, P_old: ndarray) -> tuple[float, float, float]:
...
def check_convergence(SCF_conv: float, step: int, delta_E: float, max_DP: float, RMS_DP: float, commutator: float, calculation: Calculation, silent: bool = False) -> bool:
...
def calculate_unrestricted_electronic_energy(integrals: Integrals, P_alpha: ndarray, P_beta: ndarray, J_alpha: ndarray, J_beta: ndarray, K_alpha: ndarray, K_beta: ndarray, calculation: Calculation, alpha_density: ndarray, beta_density: ndarray, weights: ndarray, e_X_alpha: ndarray, e_X_beta: ndarray, e_C: ndarray) -> tuple:
    ...
    Args:
        integrals (Integrals): Kinetic energy matrix in AO basis
...
def construct_restricted_Fock_matrix(integrals: Integrals, P: ndarray, HFX_prop: float, V_XC: ndarray) -> tuple:
    ...
    J = calculate_coulomb_matrix(P, integrals.ERI_AO)
    K = calculate_exchange_matrix(P, integrals.ERI_AO)
...
def construct_unrestricted_Fock_matrices(integrals: Integrals,  P_alpha: ndarray, P_beta: ndarray, HFX_prop: float, V_XC_alpha: ndarray, V_XC_beta: ndarray) -> tuple[ndarray, ndarray, ndarray, ndarray, ndarray, ndarray]:
    ...
    J_alpha = calculate_coulomb_matrix(P_alpha, integrals.ERI_AO)
    J_beta = calculate_coulomb_matrix(P_beta, integrals.ERI_AO)

    # Calculates the Fock exchange matrices

    K_alpha = calculate_exchange_matrix(P_alpha, integrals.ERI_AO)
    K_beta = calculate_exchange_matrix(P_beta, integrals.ERI_AO)
...
def run_restricted_SCF_cycle(step: int, E: float, P: ndarray, P_old: ndarray, P_before_damping: ndarray, DIIS_error_vector: ndarray, Fock_vector: ndarray, calculation: Calculation, molecule: Molecule, integrals: Integrals, X: ndarray, n_doubly_occ: int, silent: bool, bfs_on_grid: ndarray, bf_gradients_on_grid: ndarray, exchange_functional: callable, correlation_functional: callable, weights: ndarray) -> tuple:
    ...
    F, J, K = construct_restricted_Fock_matrix(integrals, P, calculation.HFX_prop, V_XC)

    # Calculates the DIIS error and updates the Fock and error vectors

    commutator, Fock_vector, DIIS_error_vector, _, _ = calculate_DIIS_error(F, F, P, P, integrals.S, X, DIIS_error_vector, Fock_vector, calculation)
...
def run_self_consistent_field_cycle(molecule: Molecule, calculation: Calculation, integrals: Integrals, V_NN: float, X: ndarray, guess_objects: tuple, grid_container: tuple, silent: bool) -> Output:
    ...
    Args:
        molecule (Molecule): Molecule object
        calculation (Calculation): Calculation object
        integrals (Integrals): Integrals object containing all necessary integrals
        V_NN (float): Nuclear-nuclear repulsion energy
        X (array): Fock transformation matrix
        guess_objects (tuple): Guess density matrices in AO basis (P, P_alpha, P_beta) and guess energy
        grid_container (tuple): Tuple of basis functions and gradients evaluated on integration grid and integration weights
        silent (bool): Should anything be printed
    ...
    for step in range(1, calculation.max_iter + 1):
        ...
        if reference == "RHF":
            ...
            # Runs a UHF/UKS SCF step

            E, E_old, P, P_old, P_alpha, P_beta, commutator, damping_factor, molecular_orbitals_alpha, molecular_orbitals_beta, epsilons_alpha, epsilons_beta, energy_components, F_alpha, F_beta, alpha_density, beta_density, density = run_unrestricted_SCF_cycle(step, E, P_alpha, P_old_alpha, P_beta, P_old_beta, P, P_old, P_before_damping_alpha, P_before_damping_beta, DIIS_error_vector, Fock_vector, calculation, molecule, T, V_NE, ERI_AO, S, X, molecule.n_alpha, molecule.n_beta, silent, bfs_on_grid, bf_gradients_on_grid, exchange_functional, correlation_functional, weights, integrals)
